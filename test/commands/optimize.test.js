...
test('/optimize is registered with a required autocompleted class and a choice faction option', async () => {
  const cmds = await loadCommands();
  const cmd = cmds.get('optimize');
  assert.ok(cmd, 'optimize command should be registered');
...
test('optimize autocomplete suggests roster classes and returns the class key', async () => {
  ...
  const interaction = {
    inGuild: () => false,
    options: { getFocused: () => 'hun', getString: () => null },
